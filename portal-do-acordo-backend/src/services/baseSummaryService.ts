...
type PortfolioSummaryRow = {
  credor: string;
  borderos: number | string;
  valor_entrada: number | string | null;
  processos: number | string | null;
  titulos: number | string | null;
  importados: number | string | null;
  duplicados: number | string | null;
};
...
function selectedMonths(filter: PortfolioQuery) {
  if (filter.periodos.length > 0) return [...filter.periodos];
  return filter.periodo ? [filter.periodo] : [];
}

function annualPortfolioFilter(filter: PortfolioQuery): PortfolioQuery {
  const years = Array.from(new Set(selectedMonths(filter).map((month) => month.slice(0, 4))));
  if (years.length === 0) return { ...filter, periodo: undefined, periodos: [] };
...
function buildMonthFilter(expression: string, months: string[], params: unknown[]) {
  if (months.length === 0) return '';
  const placeholders = months.map((month) => addSqlParam(params, month)).join(', ');
...
async function queryPortfolioSummary(prisma: PrismaClient, empresaId: number, filter: PortfolioQuery) {
  ...
  return prisma.$queryRawUnsafe<PortfolioSummaryRow[]>(
    `
      WITH normalized AS (
        SELECT
          b.idempresa,
...
async function queryPaymentSummary(prisma: PrismaClient, empresaId: number, filter: PortfolioQuery) {
  const months = selectedMonths(filter);
  const range = monthRange(months, filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  ...
  return prisma.$queryRawUnsafe<PaymentSummaryRow[]>(
    `
      SELECT
...
async function queryAgreementSummary(prisma: PrismaClient, empresaId: number, filter: PortfolioQuery) {
  const months = selectedMonths(filter);
  const range = monthRange(months, filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  const negociadores = NEGOTIATORS.map((negociador) => addSqlParam(params, negociador)).join(', ');
  ...
  return prisma.$queryRawUnsafe<AgreementSummaryRow[]>(
    `
      SELECT
        TRIM(COALESCE(c.grupo, 'OUTROS')) AS credor,
...
async function buildBaseSummary(filter: PortfolioQuery) {
  ...
  const valuationTicketByCreditor = new Map(
    valuationPortfolioRows
      .map((row) => {
        const processos = toNumber(row.processos);
        return [row.credor, processos > 0 ? toNumber(row.valor_entrada) / processos : 0] as const;
      })
  ...
  const agingByCreditor = activeAgingByCreditor.length > 0
    ...
    : activeBase.aging.flatMap((row) => ({ credor: 'TOTAL', faixa: row.faixa as AgingRange, processos: row.processos }));
  ...
  const agingMap = new Map<AgingRange, { faixa: AgingRange; processos: number; valorCarteira: number; recuperado: number; acordos: number }>();
  ...
  agingByCreditor.forEach((row) => {
    ...
    const faixa = row.faixa as AgingRange;
