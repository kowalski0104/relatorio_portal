...
import { addSqlParam, buildExcludedDashboardAccessFilter, buildExcludedDashboardCreditorFilter, buildSqlInFilter, getPeriodRange, parsePeriod, NEGOTIATORS } from '../utils/reportFilters';
...
type MetricRow = {
  ...
  acessosComAcordo: number | string | null;
...
type CompanyPerformanceResult = {
  current: MetricRow;
  previous: MetricRow;
  paymentNegotiators: PaymentNegotiatorRow[];
  agreementNegotiators: AgreementNegotiatorRow[];
  accessNegotiators: AccessNegotiatorRow[];
  monthlyPayments: MonthRow[];
  monthlyAgreements: MonthRow[];
  monthlyAccesses: MonthRow[];
  dailyPayments: DayRow[];
  dailyAgreements: DayRow[];
  dailyAccesses: DayRow[];
  hourlyAgreements: HourRow[];
  hourlyAccesses: HourRow[];
};
...
function yearRange(periodo?: string) {
...
function buildCredorFilter(params: unknown[], filter: DashboardResultGraphsQuery, expression: string) {
  return buildSqlInFilter(expression, filter.credores, params);
}

function accessCreditorSubquery(params: unknown[], filter: DashboardResultGraphsQuery, startParam = '$2', endParam = '$3') {
...
async function queryPaymentMetrics(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery, periodo?: string) {
  const range = getPeriodRange(periodo ?? filter.periodo);
  ...
  const rows = await prisma.$queryRawUnsafe<Array<{ recuperado: number | string | null; pagos: number | string | null }>>(
    `
      SELECT
...
async function queryAgreementMetrics(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery, periodo?: string) {
  const range = getPeriodRange(periodo ?? filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  const negociadores = buildNegotiatorList(params, filter);
  const credorFilter = buildCredorFilter(params, filter, "TRIM(COALESCE(c.grupo, 'OUTROS'))");

  const rows = await prisma.$queryRawUnsafe<Array<{ acordos: number | string | null }>>(
    `
...
async function queryAccessMetrics(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery, periodo?: string) {
  const range = getPeriodRange(periodo ?? filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  ...
  const rows = await prisma.$queryRawUnsafe<Array<{ acessos: number | string | null; acessosComAcordo: number | string | null; negociacoes: number | string | null }>>(
    `
      SELECT
...
async function querySummaryMetrics(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery, periodo?: string): Promise<MetricRow> {
    ...
    return prisma.$queryRawUnsafe<AccessNegotiatorRow[]>(
      `
        ...
        WHERE a.idempresa = $1
          ...
          AND a.data_cad < $3
          ${buildExcludedDashboardAccessFilter('a')}
          ${buildExcludedDashboardCreditorFilter('ac_credor.idcredor')}
...
async function queryMonthlyPayments(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery) {
...
async function queryMonthlyAgreements(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery) {
  const range = yearRange(filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  const negociadores = buildNegotiatorList(params, filter);
  const credorFilter = buildCredorFilter(params, filter, "TRIM(COALESCE(c.grupo, 'OUTROS'))");
...
async function queryMonthlyAccesses(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery) {
  const range = yearRange(filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  const accessRange = getPeriodRange(filter.periodo);
  const accessFilterForCreditor = { ...filter, periodo: filter.periodo };
  const baixaSubquery = accessCreditorSubquery(params, accessFilterForCreditor, addSqlParam(params, accessRange.start), addSqlParam(params, accessRange.end));
  const credorFilter = buildCredorFilter(params, filter, "TRIM(COALESCE(b.credor, 'OUTROS'))");
  const negociadorFilter = filter.negociador ? 'AND b.negociador IS NOT NULL' : '';
...
async function queryDailyPayments(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery) {
  ...
  return prisma.$queryRawUnsafe<DayRow[]>(
    `
...
async function queryHourlyAgreements(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery) {
  const range = getPeriodRange(filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  const negociadores = buildNegotiatorList(params, filter);
  const credorFilter = buildCredorFilter(params, filter, "TRIM(COALESCE(c.grupo, 'OUTROS'))");
...
async function queryHourlyAccesses(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery) {
  const range = getPeriodRange(filter.periodo);
  const params: unknown[] = [empresaId, range.start, range.end];
  const baixaSubquery = accessCreditorSubquery(params, filter);
  const credorFilter = buildCredorFilter(params, filter, "TRIM(COALESCE(b.credor, 'OUTROS'))");
...
async function queryCompanyPerformance(prisma: PrismaClient, empresaId: number, filter: DashboardResultGraphsQuery): Promise<CompanyPerformanceResult> {
...
function mergeMetric(rows: MetricRow[]) {
  const total = rows.reduce<NumericMetric>(
    (sum, row) => ({
...
function mergeNegotiators(results: CompanyPerformanceResult[]) {
  const rows = new Map<string, { negociador: string; acordos: number; acordosPagos: number; recuperado: number; acessos: number; ticketMedio: number; conversao: number }>();
...
function mergeMonths(results: CompanyPerformanceResult[]) {
  const rows = new Map<string, { mes: string; recuperado: number; acordos: number; acessos: number; conversao: number }>();
  const ensure = (mes: Date | string) => {
...
function mergeDays(results: CompanyPerformanceResult[]) {
  const rows = new Map<string, { dia: string; recuperado: number; acordos: number; acessos: number; conversao: number }>();
...
function mergeHours(results: CompanyPerformanceResult[]) {
  const rows = new Map<number, { hora: number; label: string; acessos: number; acordos: number; conversao: number }>();
  const ensure = (hora: number | string | null | undefined) => {
...
function buildPerformancePayload(results: CompanyPerformanceResult[], filter: DashboardResultGraphsQuery) {
...
async function buildPerformance(filter: DashboardResultGraphsQuery) {
  const results = await Promise.all(
    getLiveClients(filter.sistema).map(({ empresaId, query }) =>
      query((prisma) => queryCompanyPerformance(prisma, empresaId, filter))
    )
  );
...
export async function getDashboardPerformanceSummary(filter: DashboardResultGraphsQuery) {
  return getCached(cacheKey('dashboard-performance-summary', filter), CACHE_TTL.PERFORMANCE, () => buildPerformance(filter));
}
