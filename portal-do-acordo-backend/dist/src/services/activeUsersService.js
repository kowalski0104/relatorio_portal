...
exports.recordActiveUserHeartbeat = recordActiveUserHeartbeat;
exports.getActiveUsersReport = getActiveUsersReport;
...
function stringValue(value, fallback = '', maxLength = 250) {
...
function hashIp(ip) {
    ...
    return (0, crypto_1.createHash)('sha256').update(`${salt}:${ip}`).digest('hex').slice(0, 18);
}
...
function recordActiveUserHeartbeat(req) {
    const body = (req.body ?? {});
    const now = Date.now();
    const rawSessionId = stringValue(body.sessionId, '', 120);
    const sessionId = rawSessionId || (0, crypto_1.randomUUID)();
...
//# sourceMappingURL=activeUsersService.js.map