...
import { addSqlParam, buildSqlInFilter, getPeriodoRange, NEGOCIADORES } from './shared';
async function queryAcessos(prisma, empresaId, filter) {
    const periodo = getPeriodoRange(filter.periodo);
    const params = [empresaId, periodo.start, periodo.end];
    const negociadores = NEGOCIADORES.map((negociador) => addSqlParam(params, negociador)).join(', ');
...
export async function getAcessos(filter) {
    const results = await Promise.all(getLiveClients(filter.sistema).map(({ empresaId, prisma }) => queryAcessos(prisma, empresaId, filter)));
...
//# sourceMappingURL=acessosService.js.map