...
import { addSqlParam, formatMonthLabel, getLastThreeMonths, NEGOCIADORES } from './shared';
...
async function queryCustos(prisma, empresaId, start, end) {
    const baixaParams = [empresaId, start, end];
    const acordoParams = [empresaId, start, end];
    const baixaNegociadores = NEGOCIADORES.map((negociador) => addSqlParam(baixaParams, negociador)).join(', ');
    const acordoNegociadores = NEGOCIADORES.map((negociador) => addSqlParam(acordoParams, negociador)).join(', ');
    const baixasQuery = `
    SELECT
  ...
  `;
    const [baixas, acordos] = await Promise.all([
        prisma.$queryRawUnsafe(baixasQuery, ...baixaParams),
        prisma.$queryRawUnsafe(acordosQuery, ...acordoParams),
    ]);
    return { baixas, acordos };
}
export async function getCustos(filter) {
    ...
    const results = await Promise.all(getLiveClients(filter.sistema).map(({ empresaId, prisma }) => queryCustos(prisma, empresaId, firstDate, lastDate)));
...
//# sourceMappingURL=custosService.js.map