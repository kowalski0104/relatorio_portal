...
async function buildCosts(filter) {
    ...
    acordos.forEach((row) => {
        ...
        if (acordosByMonth.has(key))
            acordosByMonth.set(key, acordosByMonth.get(key) + Number(row.acordos));
    });
...
async function getCosts(filter) {
    return (0, cache_1.getCached)((0, cache_1.cacheKey)('costs', filter), cache_1.CACHE_TTL.COSTS, () => buildCosts(filter));
}
