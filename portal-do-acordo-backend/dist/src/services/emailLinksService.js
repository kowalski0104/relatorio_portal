...
exports.EmailLinksPublicError = exports.DEFAULT_PUBLIC_API_BASE_URL = exports.DEFAULT_DESTINATION_URL = void 0;
exports.bulkGenerateEmailLinks = bulkGenerateEmailLinks;
const crypto_1 = __importDefault(require("crypto"));
const mssql_1 = __importDefault(require("mssql"));
exports.DEFAULT_DESTINATION_URL = 'https://portaldoacordo.com.br';
exports.DEFAULT_PUBLIC_API_BASE_URL = 'https://portal-relatorio-api-aucpaha6dphdhegp.canadacentral-01.azurewebsites.net';
const SCHEMA_CACHE_TTL_MS = 5 * 60 * 1000;
class EmailLinksPublicError extends Error {
    statusCode;
...
exports.EmailLinksPublicError = EmailLinksPublicError;
let pool = null;
let poolPromise = null;
let cachedSchema = null;
async function bulkGenerateEmailLinks(input) {
...
async function upsertEmailLink(pool, schema, input, item, destinationUrl, publicApiBaseUrl) {
    const email = cleanString(item.email);
    if (!email)
        throw new EmailLinksPublicError('Item sem email obrigatorio.', 400);
    const processo = cleanString(item.processo);
...
async function getEmailEnviosSchema(pool) {
    if (cachedSchema && cachedSchema.expiresAt > Date.now())
...
function ensureMinimumSchema(schema) {
    const required = [
        ['token'],
        ['url_destino'],
...
async function findExistingToken(pool, schema, input, item, persistedUniqueKey) {
    ...
    for (const conditions of buildFallbackMatchPlans(schema, input, item)) {
        const request = pool.request();
        conditions.forEach((condition) => request.input(condition.paramName, mssql_1.default.NVarChar(4000), condition.value));
        const result = await request.query(`
      SELECT TOP 1 ${columnRef(tokenColumn)} AS token
      FROM ${schema.tableRef}
...
async function createUniqueToken(pool, schema, prefix) {
    const tokenColumn = requireColumn(schema, 'token');
    for (let attempt = 0; attempt < 5; attempt += 1) {
        const token = `${prefix}-${crypto_1.default.randomUUID().toUpperCase()}`;
        const request = pool.request();
...
async function updateExistingEmailLink(pool, schema, input, item, destinationUrl, rawUniqueKey, persistedUniqueKey, token) {
...
async function insertEmailLink(pool, schema, input, item, destinationUrl, rawUniqueKey, persistedUniqueKey, token) {
...
function buildColumnValues(schema, input, item, destinationUrl, rawUniqueKey, persistedUniqueKey, token, mode) {
...
function buildFallbackMatchPlans(schema, input, item) {
...
function normalizedEquals(column, paramName, value) {
    return {
...
function normalizedAnyEquals(columns, paramName, value) {
    return {
...
function buildLogicalUniqueKey(input, item) {
...
function formatUniqueKeyForColumn(rawUniqueKey, column) {
    if (!column || column.maxLength === null || column.maxLength === -1 || rawUniqueKey.length <= column.maxLength) {
        return rawUniqueKey;
    }
    const hash = crypto_1.default.createHash('sha256').update(rawUniqueKey).digest('hex');
    const hashed = `sha256:${hash}`;
    if (hashed.length <= column.maxLength)
        return hashed;
...
function buildPayloadJson(input, item, rawUniqueKey, now) {
...
function normalizeDestinationUrl(value) {
    const raw = cleanString(value) ?? exports.DEFAULT_DESTINATION_URL;
    let url;
    try {
        url = new URL(raw);
    }
...
function getPublicApiBaseUrl() {
    const raw = cleanString(process.env.PUBLIC_API_BASE_URL) ?? exports.DEFAULT_PUBLIC_API_BASE_URL;
    try {
        ...
        return exports.DEFAULT_PUBLIC_API_BASE_URL;
    }
}
function buildTrackingLink(publicApiBaseUrl, token) {
    return `${publicApiBaseUrl}/r/${encodeURIComponent(token)}`;
}
function prefixForCredor(value) {
    const normalized = normalizeForPrefix(value);
    if (normalized.includes('SISTH'))
        return 'sisth';
    if (normalized.includes('CONSULTH'))
        return 'consulth';
    return 'portal';
}
...
function setValue(values, column, value) {
    if (!column || value === undefined)
        return;
    values.set(column.name.toLowerCase(), { column, value });
}
function getColumn(schema, ...candidates) {
...
function getColumns(schema, ...candidates) {
    const found = new Map();
    candidates.forEach((candidate) => {
        const column = getColumn(schema, candidate);
        if (column)
            found.set(column.name.toLowerCase(), column);
    });
...
function requireColumn(schema, ...candidates) {
    ...
    if (!column)
        throw new EmailLinksPublicError(`Campo obrigatorio nao encontrado: ${candidates.join('/')}.`);
    return column;
}
function addTypedInput(request, paramName, column, value) {
    const type = column.dataType;
    if (type === 'bit') {
        request.input(paramName, mssql_1.default.Bit, Boolean(value));
        return;
    }
    if (isDateType(type)) {
        request.input(paramName, mssql_1.default.DateTime2, value instanceof Date ? value : new Date(String(value)));
        return;
    }
...
function stringSqlType(column) {
    const maxLength = column.maxLength;
    if (!maxLength || maxLength === -1 || maxLength > 4000)
        return mssql_1.default.NVarChar(mssql_1.default.MAX);
    return mssql_1.default.NVarChar(maxLength);
}
...
//# sourceMappingURL=emailLinksService.js.map